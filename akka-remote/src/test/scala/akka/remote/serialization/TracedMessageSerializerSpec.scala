...
import akka.trace.{ ContextOnlyTracer, TracedMessage, Tracer }
...
class TracedMessageSerializerSpec extends AkkaSpec(TracedMessageSerializerSpec.testConfig) {
  ...
  "TracedMessageSerializer" must {
    ...
    "resolve serializer for TracedMessage" in {
      serializer.getClass must be(classOf[TracedMessageSerializer])
    }
    ...
    "serialize and deserialize between different numbers of tracers" in {
      ...
      val expectedMulti = TracedMessage("hello", Vector(extendedSystem.tracer.getContext, Tracer.emptyContext))

      singleToMulti must be(expectedMulti)
      multiToSingle must be(single)
